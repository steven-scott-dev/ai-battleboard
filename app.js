// ============================================================
// MODEL DATA
// This is the core data store for the app. Each key is a
// unique model ID, and each value is an object containing
// everything the UI needs to display for that model.
// ============================================================
const MODELS = {
  ...
  "gpt-5.1": {
    ...
    priceIndex: 2,          // Lower = cheaper (used for comparison logic, not display)
    speed: "Fast",          // Human-readable speed label shown in the card
    speedIndex: 2,          // Lower = faster (used for comparison logic)
    rating: 9.4,            // Overall rating out of 10
    offlineSupport: false,  // Whether the model can run locally/offline
    bestUseCases: [
    ...
    // Niche scores used in "Best For You" section comparisons
  ...
  "llama-3": {
    ...
    speedIndex: 2,
    rating: 8.5,
    offlineSupport: true, // Only model in the demo that supports offline/local use
    bestUseCases: [
...
// ============================================================
// DEMO MODEL IDS
// Only these models appear in the free version's dropdowns.
// To add more models to the demo, add their ID here.
// The full MODELS object above can be expanded for Pro.
// ============================================================
...
// Full flat array of all models — available for future Pro use
const MODEL_LIST = Object.values(MODELS);

// Grab the two dropdown elements from the DOM
const modelASelect = document.getElementById("modelA");
const modelBSelect = document.getElementById("modelB");

// ============================================================
// POPULATE MODEL DROPDOWNS
// Clears and rebuilds both <select> elements using only the
// models in DEMO_MODEL_IDS. Sorted alphabetically by name.
// Sets default selections to GPT-5.1 and Claude 3.5.
// ============================================================
function populateModelSelects() {
  ...
  // Add a disabled placeholder as the first option
  ...
  // Filter MODELS down to only demo IDs, then sort A–Z
  const demoModels = DEMO_MODEL_IDS
    .map((id) => MODELS[id])
    .filter(Boolean); // .filter(Boolean) removes any undefined entries if an ID doesn't match

  const sorted = demoModels.sort((a, b) => a.name.localeCompare(b.name));

  // Build and append an <option> for each model in both selects
  ...
  // Set default selections if those models are in the demo set
...
// ============================================================
// RENDER LIST HELPER
// Converts an array of strings into an HTML <ul> list.
// If asUl is false, returns a comma-separated string instead.
// Returns "—" if the list is empty or undefined.
// ============================================================
...
// ============================================================
// COMPARE SPEED
// Returns a plain-English string describing which model is faster.
// Uses speedIndex — lower number means faster response.
// ============================================================
...
// ============================================================
// COMPARE PRICE
// Returns a plain-English string describing which model is cheaper.
// Uses priceIndex — lower number means cheaper.
// ============================================================
...
// ============================================================
// ANIMATE CARD
// Triggers the slide-in animation on a model card.
// Removes .show, forces a reflow (offsetWidth trick), then
// re-adds .show so the CSS transition fires again cleanly.
// ============================================================
function animateCard(cardElement) {
  cardElement.classList.remove("show");
  void cardElement.offsetWidth; // Forces the browser to recalculate layout, resetting the transition
  cardElement.classList.add("show");
}

// ============================================================
// RENDER MODEL CARD
// Takes a card ID prefix ("modelA" or "modelB") and a model
// object, then fills in all the card's DOM elements with data.
// ============================================================
function renderModelCard(cardIdPrefix, model) {
  // Set the model name and rating in the card header
  document.getElementById(cardIdPrefix + "Name").textContent = model.name;
  document.getElementById(cardIdPrefix + "Rating").textContent =
    model.rating.toFixed(1); // toFixed(1) ensures one decimal place e.g. "9.0" not "9"

  // Build and inject the tag chips
  const tagsContainer = document.getElementById(cardIdPrefix + "Tags");
  tagsContainer.innerHTML = ""; // Clear previous tags before re-rendering

  model.tags.forEach((tag) => {
    const span = document.createElement("span");
    span.classList.add("tag");

    // Highlight key tags (coding, writing, research) with accent styling
  ...
  // Fill in the three pricing values
  ...
  // Handle offline support — update text AND the colored dot indicator
  ...
  if (model.offlineSupport) {
    ...
    offlineDotEl.classList.add("offline-yes"); // Green dot
  } else {
    offlineTextEl.textContent = "No (cloud-only in this demo)";
    offlineDotEl.classList.remove("offline-yes");
    offlineDotEl.classList.add("offline-no"); // Red dot
  }

  // Fill in the text sections using the renderList helper
  ...
  // Grab the card's outer element and trigger its entrance animation
  const cardElement =
    cardIdPrefix === "modelA"
...
// ============================================================
// NICHE WINNER
// Compares two models on a specific niche (coding, writing, research).
// Returns an object with the winner's name and model reference.
// Returns "Tie" if scores are equal.
// ============================================================
function nicheWinner(modelA, modelB, nicheKey) {
  const aScore = modelA.niches?.[nicheKey] ?? 0; // ?. and ?? safely handle missing data
...
// ============================================================
// RENDER BEST FOR YOU
// Fills in the 4 "Best For You" boxes at the bottom of the page:
// cheapest, fastest, best for coding, best for writing & research.
// All comparisons use demo data — not official benchmarks.
// ============================================================
function renderBestForYou(modelA, modelB) {
  ...
  // Cheapest — lower priceIndex wins
  const cheaper =
  ...
  // Best for coding — uses nicheWinner helper
  ...
  // Best for writing & research — averages both niche scores together
  const aWR =
...
// ============================================================
// BUILD SHARE SUMMARY
// Generates a plain-text summary of the comparison that users
// can copy and paste into chats, docs, or social posts.
...
function buildShareSummary(modelA, modelB) {
  ...
  // Calculate writing + research average for each model
  const aWR =
    ((modelA.niches?.writing ?? 0) + (modelA.niches?.research ?? 0)) / 2;
  const bWR =
    ((modelB.niches?.writing ?? 0) + (modelB.niches?.research ?? 0)) / 2;
  const wr =
    aWR === bWR ? null : aWR > bWR ? modelA : modelB;

  // Build the summary as an array of lines, then join with newlines
  return [
    `AI Battleboard — ${modelA.name} vs ${modelB.name}`,
    "",
    `• Rating: ${modelA.name} ${modelA.rating.toFixed(1)}/10 vs ${modelB.name} ${modelB.rating.toFixed(1)}/10`,
...
// ============================================================
// SHOW TOAST
// Briefly shows a small notification at the bottom-right.
// Adds the .show class (CSS handles the animation), then
...
// Grab the buttons and share status text element from the DOM
...
// ============================================================
// HANDLE COMPARE
// Runs when the "Compare Now" button is clicked.
// Reads both dropdown values, looks up the model data,
// and re-renders both cards + the Best For You section.
// ============================================================
function handleCompare() {
  const idA = modelASelect.value;
  const idB = modelBSelect.value;

  // Guard: do nothing if either dropdown is empty or has an invalid ID
  if (!idA || !idB || !MODELS[idA] || !MODELS[idB]) return;

  const modelA = MODELS[idA];
  const modelB = MODELS[idB];

  // Re-render both model cards with fresh data
  renderModelCard("modelA", modelA);
  renderModelCard("modelB", modelB);

  // Update the Best For You section
  renderBestForYou(modelA, modelB);

  // Update the share status text to reflect the current comparison
  shareStatus.textContent = `Ready to share: ${modelA.name} vs ${modelB.name} summary.`;
}

// ============================================================
// HANDLE SHARE
// Runs when the "Copy Summary" button is clicked.
// Uses the Clipboard API if available, falls back to a
// prompt() dialog for older browsers.
// ============================================================
function handleShare() {
  const idA = modelASelect.value;
  const idB = modelBSelect.value;

  // Guard: prompt user to pick models if nothing is selected
  ...
  // Try modern clipboard API first, fall back to prompt() if unavailable
  if (navigator.clipboard && navigator.clipboard.writeText) {
    ...
    window.prompt("Copy this comparison summary:", text); // Fallback for older browsers
  }
}

// ============================================================
// INIT
// Runs on page load — populates the dropdowns and immediately
// renders the default comparison (GPT-5.1 vs Claude 3.5).
// ============================================================
